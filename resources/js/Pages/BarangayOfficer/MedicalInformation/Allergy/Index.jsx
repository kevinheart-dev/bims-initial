...
export default function Index({ allergies, puroks, queryParams }) {
    const breadcrumbs = [
        ...
        { label: "Allergies", showOnMobile: true },
    ];
    ...
    const searchFieldName = (field, value) => {
        ...
        router.get(route("allergy.index", queryParams));
    };
    ...
    const allColumns = [
        ...
        { key: "age", label: "Age" },
        { key: "sex", label: "Sex" },
        { key: "allergy_name", label: "Allergy Name" },
        { key: "reaction_description", label: "Description" },
    ...
    const hasActiveFilter = Object.entries(queryParams || {}).some(
        ([key, value]) =>
            ["purok", "sex", "age_group", "allergy"].includes(key) &&
            value &&
            value !== ""
    );
    ...
    const columnRenderers = {
        ...
        allergy_name: (row) => row.allergy_name ?? "—",

        reaction_description: (row) => (
                ...
                actions={[
                    {
                        ...
                        onClick: () => handleView(allergy.resident?.id),
                    },
                    {
                        label: "Edit",
                        icon: <SquarePen className="w-4 h-4 text-green-500" />,
                        onClick: () => {
                            router.get(route("allergies.edit", allergy.id));
                        },
                    },
                    {
                        label: "Delete",
    ...
    const confirmDelete = () => {
        router.delete(route("allergy.destroy", disabilityToDelete), {
            onError: (errors) => {
    ...
    return (
        <AdminLayout>
            ...
            <div>
                ...
                {/* <pre>{JSON.stringify(allergies, undefined, 2)}</pre> */}
                        ...
                        <div className="bg-white border border-gray-200 shadow-sm rounded-xl sm:rounded-lg p-4 m-0">
                            ...
                            {showFilters && (
                                <FilterToggle
                                    ...
                                    puroks={puroks}
                                    showFilters={true}
                                    clearRouteName="allergy.index"
                                    clearRouteParams={{}}
                                />
                            )}
                            <DynamicTable
                                passedData={allergies}
                                allColumns={allColumns}
