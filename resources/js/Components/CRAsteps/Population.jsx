...
export default function Population() {
    ...
    const getHouseGrandTotal = () => {
        return craData.houses?.reduce(
            (sum, house) => sum + (house.oneFloor || 0) + (house.multiFloor || 0),
            0
        );
    };
    ...
    return (
        <div className="space-y-4">
            <Accordion title="A. Information on Population and Resident">
                ...
                {/* Age Group Table */}
                <section>
                    <h2 className="text-lg font-semibold mb-3 mt-6">Population according to Age</h2>
                            ...
                            <table className="w-full border text-sm h-full">
                                ...
                                <tbody>
                                    {craData.houses?.map((house, idx) => (
                                        <tr key={idx}>
                                            <td className="border px-2 py-1">{house.houseType}</td>
                                            <td className="border px-2 py-1">
                                                <input
                                                    ...
                                                    onChange={(e) =>
                                                        updateHouse(idx, "oneFloor", e.target.value)
                                                    }
                                                />
                                            </td>
                                            <td className="border px-2 py-1">
                                                <input
                                                    type="number"
                                                    className="w-full border p-1"
                                                    value={house.multiFloor ?? ""}
                                                    onChange={(e) =>
                                                        updateHouse(idx, "multiFloor", e.target.value)
                                                    }
                                                />
                                            </td>
                                            <td className="border px-2 py-1 font-semibold text-center bg-gray-50">
                                                {(house.oneFloor || 0) + (house.multiFloor || 0)}
                                            </td>
                                        </tr>
                                    ))}
                                </tbody>
