    ...
    return (
        <header className="fixed top-0 left-0 w-full flex justify-between items-center text-[#093a7b] py-2 px-8 md:px-32
            ...
            <div className="relative flex items-center gap-3">
                <a href="#">
                    <img src="/images/city-of-ilagan.png" alt="" className="w-12 hover:scale-120 transition-all" />
                </a>
                <h1 className="text-[#004aad] font-montserrat text-xl font-black">iBMIS</h1>
            </div>

            {/* Desktop Menu */}
            <ul className="hidden xl:flex items-center gap-12 font-md text-sm">
            </ul>
            ...
            {/* Mobile Menu Toggle */}
            <div className="xl:hidden text-4xl cursor-pointer" onClick={handleClick}>
                <RiMenu4Line />
            </div>
            ...
            {isMenuOpen && (
                <ul className="absolute top-16 right-8 bg-white shadow-lg p-4 rounded-lg flex flex-col gap-4 text-blue-500">
                    {auth.user ? (
                        ...
                        <>
                            <li>
                                <Link href={route('login')} className="hover:text-blue-700">
                                    Log In
                                </Link>
                            </li>
                            <li>
                                <Link href={route('register')} className="hover:text-green-700">
                                    Register
                                </Link>
                            </li>
                        </>
