        ...
        <div className="relative overflow-hidden bg-[#f8faff] font-montserrat min-h-screen">
            ...
            <div className="w-full max-w-7xl mx-auto px-6 md:px-20 lg:px-2 pb-20">
                ...
                <div className="relative z-10 flex flex-col items-start justify-center py-10 space-y-2">
                    ...
                    <p className="text-[#093a7b] text-sm sm:text-base leading-relaxed text-left max-w-8xl">
                        This section provides an overview of the barangay's employment status.
                        The total recorded population is{" "}
                        <strong>{total.toLocaleString()}</strong> residents. Among them,{" "}
                        <strong>{breakdown.employed?.count || 0}</strong> (
                        {breakdown.employed?.percentage || 0}%) are employed,{" "}
                        <strong>{breakdown.self_employed?.count || 0}</strong> (
                        {breakdown.self_employed?.percentage || 0}%) are self-employed,{" "}
                        <strong>{breakdown.student?.count || 0}</strong> (
                        {breakdown.student?.percentage || 0}%) are students,{" "}
                        <strong>{breakdown.under_employed?.count || 0}</strong> (
                        {breakdown.under_employed?.percentage || 0}%) are under-employed, and{" "}
                        <strong>{breakdown.unemployed?.count || 0}</strong> (
                        {breakdown.unemployed?.percentage || 0}%) are unemployed.
                        This summary highlights the distribution of the workforce and education
                        engagement across the barangay.
                    </p>
                </div>
