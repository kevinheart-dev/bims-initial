...
<html>
<head>
    <title>{{ $barangayName }} - PWD Summary ({{ $year ?? now('Asia/Manila')->year }})</title>
    <style>
...
<h2>{{ $barangayName }} Persons With Disability Summary</h2>
...
<tbody>
@forelse($disabilities as $index => $d)
<tr>
    <td>{{ $index + 1 }}</td>
    <td style="text-align:left;">{{ $d['FullName'] ?? 'N/A' }}</td>
    <td>{{ $d['Age'] ?? 'N/A' }}</td>
    <td>{{ $d['Sex'] ?? 'N/A' }}</td>
    <td style="text-align:left;">{{ $d['Disability Type'] ?? 'N/A' }}</td>
    <td>{{ $d['PWD ID Number'] ?? 'N/A' }}</td>
    <td>{{ $d['Purok Number'] ?? 'N/A' }}</td>
</tr>
@empty
<tr>
...
<tr class="totals">
    <td colspan="7" style="text-align:left; font-style:italic;">
        ...
        Total Records: {{ $total ?? $disabilities->count() }}
    </td>
</tr>
</tfoot>
</table>
...
<table class="summary-table">
    <tr><th>Total PWDs</th><td>{{ $total ?? $disabilities->count() }}</td></tr>
    <tr><th>Male</th><td>{{ $summary['male'] ?? 0 }}</td></tr>
    <tr><th>Female</th><td>{{ $summary['female'] ?? 0 }}</td></tr>
    <tr><th>With PWD ID</th><td>{{ $summary['with_id'] ?? 0 }}</td></tr>
    <tr><th>Without PWD ID</th><td>{{ $summary['without_id'] ?? 0 }}</td></tr>
</table>
