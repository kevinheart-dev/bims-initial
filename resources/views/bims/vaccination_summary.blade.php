...
<html>
<head>
    <title>{{ $barangayName }} - Vaccination Records Summary ({{ $year ?? now('Asia/Manila')->year }})</title>
    <style>
...
<h2>{{ $barangayName }} Vaccination Records Summary</h2>
...
<tr>
    <td>{{ $index + 1 }}</td>
    <td style="text-align:left;">{{ $v['FullName'] ?? 'N/A' }}</td>
    <td>{{ $v['Age'] ?? 'N/A' }}</td>
    <td>{{ $v['Sex'] ?? 'N/A' }}</td>
    <td>{{ $v['Vaccine'] ?? 'N/A' }}</td>
    <td>{{ $v['Vaccination Date'] ?? 'N/A' }}</td>
    <td>{{ $v['Purok Number'] ?? 'N/A' }}</td>
</tr>
@empty
<tr>
...
<tr class="totals">
    <td colspan="7" style="text-align:left; font-style:italic;">
        ...
        Total Records: {{ $total ?? $vaccinations->count() }}
    </td>
</tr>
</tfoot>
</table>
    ...
    <table class="summary-table">
        <tr><th>Total Vaccinated</th><td>{{ $summary['total'] }}</td></tr>
        <tr><th>Male</th><td>{{ $summary['male'] }}</td></tr>
        <tr><th>Female</th><td>{{ $summary['female'] }}</td></tr>

        @foreach($summary['vaccines'] as $vaccine => $count)
            <tr>
                <th>{{ ucfirst($vaccine) }}</th>
                <td>{{ $count }}</td>
            </tr>
        @endforeach
    </table>
