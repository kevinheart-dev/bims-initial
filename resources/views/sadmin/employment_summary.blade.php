...
<html>
<head>
    <title>Employment Summary - {{ $year ?? 2025 }}</title>
    <style>
        body {
...
<body>
    ...
    <table>
        <thead>
            <tr>
                ...
                @foreach($employmentCategories as $cat)
                    <th>{{ ucwords(str_replace('_', ' ', $cat)) }}</th>
                @endforeach
                <th>Total</th>
            </tr>
        </thead>
        <tbody>
            @php $index = 1; @endphp
            @foreach($employmentPerBarangay as $barangayName => $data)
                <tr class="{{ $barangayName === 'Total' ? 'totals' : '' }}">
                    ...
                    @foreach($employmentCategories as $cat)
                        <td>{{ $data[$cat] ?? 0 }}</td>
                    @endforeach
                    <td>{{ $data['total'] ?? array_sum($data) }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
